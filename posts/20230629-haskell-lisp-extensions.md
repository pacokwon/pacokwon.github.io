...
또 하스켈이 이론적인 기반이 탄탄하다 보니 다양한 패턴들에 대한 내장 함수들이 많은 듯했다. 나름 내 아이디어로 내가 알고 있는 함수들을 이렇게 저렇게 조합하고 나니, 하스켈 language 서버가 처음 들어보는 내장함수를 내밀며 "이 함수 하나를 써서 네가 방금 쓴 엉망 코드를 리팩토링하지 않겠니?"라는 제안을 받아본 것이 꽤 여러번이다.
